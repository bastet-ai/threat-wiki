...
- [@7nohe/openapi-react-query-codegen npm compromise: 10 malicious versions published through an exposed issue-comment-triggered release workflow using GitHub Actions OIDC / npm Trusted Publishing — preinstall + binding.gyp payloads download Bun and steal GitHub / cloud / CI credentials; OX + JFrog now confirm a "Trinitite" Shai-Hulud variant: two-wave release, a Python-`conditions` binding.gyp command that survives --ignore-scripts, XOR-9/AES-GCM loader into Bun v1.4.0, new campaign strings, and a live token-revocation trap that wipes the home dir (StepSecurity + OX + JFrog)](ops/7nohe-openapi-react-query-codegen-npm-compromised-august-2026.md)
