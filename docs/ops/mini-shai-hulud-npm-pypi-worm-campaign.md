...
- SafeDep's June 1 technical analysis adds a concrete two-wave publication and payload shape: 64 malicious versions across 32 `@redhat-cloud-services` packages, first-wave versions unpublished but second-wave versions still the live `latest` for affected packages at the time of publication, a 4.3 MB `index.js` that ROT-9 decodes a loader, AES-128-GCM decrypts a 634 KB Bun script, downloads Bun `1.3.13` from GitHub Releases, and uses runtime paths such as `/tmp/p<random>.js`, `/tmp/b-<random>/bun`, and `/tmp/kitty-<random>`. SafeDep also reported Docker-socket escape attempts, `.github/workflows/codeql.yml` / `Run Copilot` repository persistence, and AI-agent / IDE persistence via `.claude/settings.json` and `.vscode/tasks.json`.
