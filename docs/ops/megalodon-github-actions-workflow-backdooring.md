...
The payload lived in GitHub Actions workflow files rather than application runtime code. The workflows decoded and executed base64-encoded bash that exfiltrated CI environment variables, cloud credentials, SSH keys, source-code secrets, GitHub Actions OIDC tokens, and other runner-accessible material to `216.126.225[.]129:8443`.
...
- CI workflow files are high-impact backdoor locations because a small YAML change can run in a secret-bearing environment, request `id-token: write`, and reach cloud metadata services.
...
## Notable Tiledesk package propagation
...
- Workflow paths: newly added `.github/workflows/ci.yml` or unexpected replacement of existing build/publish workflows.
