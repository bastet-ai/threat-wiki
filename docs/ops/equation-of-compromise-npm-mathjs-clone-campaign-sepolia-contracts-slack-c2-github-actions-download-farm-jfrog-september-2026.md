...
## <a id="september-22-checkmarx-runtime-loader-overlap"></a>September 22 follow-up: Checkmarx publishes the RUNTIME-LOADER twin of this campaign's btree family — three names overlap the farm's own target list, and the €230k price tag lands

The Hacker News ran Checkmarx's teardown of **`indexed-btree`** on Sep 22 (full text captured by this wiki): a malicious npm package "mimicking the legit `sorted-btree`" that **carries no `preinstall`/`postinstall` at all** — the loader is concealed inside **`BTree.prototype.set()`**, fires `sharedLoad.min.js` at application runtime, fingerprints the host, beacons to a **hard-coded Slack channel AND a Telegram bot**, pulls next-stage encrypted blobs via **EtherHiding from a Sepolia testnet contract** (deployed by the operator), merges them into stage two, and finally **deletes its own malicious artifacts and REMOVES THE TRIGGER FROM THE PACKAGE CODE** to cover tracks. Checkmarx names the same operation's other removed packages: `ordered-kv-index`, `btree-leaderboard`, `priority-slot-queue`, `btree-range-store`, `btree-core`, `btree-time-index`, `btree-lru-cache`, `neighbor-key-map`, `sliding-score-window`, `mutex-forge`. First upload of `indexed-btree`: Jun 18, 2026 by npm user `charlessadler25`; Checkmarx estimates the operation earned **€230,933.57 (109 ETH)**. Checkmarx's framing, echoed by SOCRadar's CISO in the same piece: this is attackers **migrating off lifecycle scripts in direct response to npm 12's default script-blocking** — the exact adaptation this wiki's [npm install explicit-trust controls pattern page](../patterns/npm-install-explicit-trust-controls.md) predicted ("runtime invocation payloads hidden behind normal-looking API calls").

**This wiki's registry forensics on the same names (Sep 22, ~11:30 UTC):** every one of the eleven names in Checkmarx's list is an **`0.0.1-security` security holder, and the holders were batch-created inside a ~40-second window on Sep 3, 2026 (10:12:39–10:13:17 UTC)** — `btree-core` 10:12:31, `btree-lru-cache` 10:12:39, `btree-range-store` 10:12:44, `btree-time-index` 10:12:48, `neighbor-key-map` 10:12:52, `sliding-score-window` 10:13:09, `indexed-btree` 10:13:17 (`mutex-forge` holder predates: Aug 13). The OSV/GHSA records for the family predate this wiki's window entirely: `indexed-btree` = `MAL-2026-15888` / `GHSA-f3xx-wvxc-5vh4` (versions 2.1.1–2.1.2, published Sep 3, Amazon Inspector finder), `btree-core` = `MAL-2026-15873` / `GHSA-wj64-2cmx-c3j4` (3.2.1–3.2.4), `ordered-kv-index` = `MAL-2026-15891` / `GHSA-r948-p23m-ffg3`. **Reading: the neutralization happened on Sep 3 in one automated batch; Checkmarx's writeup landed nineteen days later.** The advisory-to-publication lag is the story's own footnote: consumers were warned Sep 3–4, the blog cycle is Sep 22.

**The cross-vendor join (recorded carefully):** three names sit on BOTH lists — `indexed-btree`, `btree-core`, and `mutex-forge` appear in Checkmarx's runtime-loader operation AND in JFrog's download-farm scheduling history (`indexed-btree` 38.0M, `btree-core` 37.7M fake downloads) AND in JFrog's malicious-versions IoC table (`indexed-btree 2.1.2`, `mutex-forge/-thread/-core/-plus/-lite`, `sort-btree`, `ordered-btree`). The OSV version ranges match the JFrog table (`indexed-btree` 2.1.1–2.1.2). On name + version + Sepolia usage, this is at minimum the same operator's package inventory being inflated by the farm and weaponized with the runtime loader; we record the overlap, not a proven single-operator identity. `charlessadler25` joins the JFrog account-persona pile (zero-follower disposable identities).

**Durable reads added by this section:**
1. **Post-npm-12 execution migration is no longer hypothetical.** The first named, dated case of a campaign moving its loader from lifecycle hooks into a normal-looking library method (`BTree.prototype.set`) with self-erasure of the trigger — install-time scanning and script approval are structurally blind to it. Runtime/behavioral detection is the countermeasure, exactly as the pattern page argues.
2. **Self-erasing triggers make the time block the only timeline.** The package deletes its own malicious code after detonation; only the registry `time` document and the tarball shas in the GHSAs prove what ran. Same lesson as `@vite-tab/tabui` and the `test-react-app-*` trio: neutralization destroys the exposure record, so pull metadata before concluding.
3. **Money changes the triage frame.** €230k / 109 ETH makes this the first npm supply-chain operation on-wiki with a stated realized profit — worth citing next to "download inflation is a weapon" to justify runtime-behavior tooling budgets.

**Same-day (fifteenth sweep, this wiki ~11:25–11:45 UTC):** KEV **unchanged 2026.09.21 / 1,717** — still no Sep 22 batch as of ~11:26 UTC (kernel trio attribution-less into day five, quartet still unlisted, no Veeam CVE-2026-32996 row yet). Farm status re-checked: `andrewstory18` / `davidbabcock96` / `azlanrahman322-creator` all still return 200 on GitHub; `events-sync` still `1.2.0`, `quick-events` `2.1.4`, `matrixflow-js` `3.2.2`, `matrixhub` `6.15.1`, `secure-library-loader` `0.1.3` — every non-trio campaign name still live with live versions (tenth confirmation of removal ≠ remediation). THN's PolinRider paragraph in the same piece is Socket's Sep 17 follow-up already on-wiki (no new facts).
