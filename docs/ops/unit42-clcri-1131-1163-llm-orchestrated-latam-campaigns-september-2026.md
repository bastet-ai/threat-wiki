...
- **September 20 sweep addition — GTIG names the Brazilian-financial side: BREEZE COMET.** GTIG's first full write-up (Sep 1, 2026, captured this sweep) of **BREEZE COMET (formerly UNC5669)** states its activity **overlaps with reporting publicly reported as Plump Spider and SHADOW-AETHER-064** — i.e., a major vendor now places its own named cluster across the same Brazilian payment-switch intrusion space this page covers, with a full tool taxonomy (COBALTSPIN Rust reverse-SOCKS5-over-WebSocket tunneler, LIGHTPAINT Java VPN-persistence backdoor, MILDFROST in-JVM DNS-tunnel fallback C2, KICKPLATE Nim persistence backdoor, BOATBEAM cookie-gated fake-IIS Go backdoor, REALBREEZE LDAP bruter), rogue-hardware retail footholds, compromised Brazilian + Nigerian/Paraguayan/Ghanaian/Venezuelan municipal-website staging/C2, and LLM-generated ops scripts (Portuguese comments, verbose step banners). GTIG's payoff metric: **two waves of hundreds of fraudulent transactions within 24–48 h** of reaching core financial applications. Still no operator identity from any of the four labels (BREEZE COMET / Plump Spider / SHADOW-AETHER-064 / CL-CRI-1163) — see [BREEZE COMET page](breeze-comet-gtig-brazilian-financial-switch-intrusions-rust-nim-go-backdoor-suite-september-2026.md).
