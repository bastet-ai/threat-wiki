...
The day after this batch was KEV-listed, researcher Asim Manizada published working exploit code for **four additional local-root kernel flaws** (The Hacker News, Sep 18, 2026): **DirtyAH6 (CVE-2026-80844**, IPsec AH6/IPv6), **TUNderflow (CVE-2026-81000**, TUN/TAP), **PPPoEject (CVE-2026-68121**, PPPoE), **DiagSpill (CVE-2026-74469**, SCTP `sctp_diag`). All four are fixed upstream; reported mid-July under a coordinated hold with distros; no ITW use reported; exploits are build-tuned and crash-prone. The connection to this page is the precondition overlap: **three of the four require unprivileged user namespaces**, and DiagSpill requires only the SCTP module — exactly the userns/netns surface this page's class-controls section tells shared hosts and CI/runner operators to gate. Read jointly with the KEV batch: CISA's Sep 18 "active across kernel exploit space broadly" grading and a same-week publication of four fresh LPE exploits mean the public weaponization pipeline for kernel LPEs is running regardless of any single CVE's KEV status. The same class controls (disable/gate unprivileged userns, deny `socket(AF_ALG)`, drop `CAP_NET_ADMIN` in userns, blacklist `sctp` where unused) close all four ahead of errata. Monitor: KEV additions for any of the four, distro-errata rollout, copycat publication against the KEV trio (this page's standing watch item).
