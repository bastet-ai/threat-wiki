...
## <a id="september-22-oracle-redis-rebuild"></a>September 22 follow-up: the same design reappears as `oracle-redis` → `redis-type-intel` / `pino-ulid` — same verbatim-copy entry package, same unused-caret arming trick, same name-fragment grammar (OSV `MAL-2026-16390`, Amazon Inspector)

Amazon Inspector's Sep 22 OSV batch (published 18:08 UTC) carries **`oracle-redis` 5.11.3**: a verbatim copy of `ioredis`'s README, source tree and repository URL under a squatted name, declaring exactly two unused caret dependencies never imported by any shipped code — **`redis-type-intel` ^1.10.5** and **`pino-ulid` ^2.12.2** — i.e. the entry package is clean by construction and any harm arrives through the transitive install. That is this chain's design item-for-item: verbatim-copy entry layer, manifest-only arming via a later-added caret range, name fragments cloned from real-ecosystem vocabulary (`redis-type-*`, `*-ulid` beside the cluster's own `redis-type-xyz` / `ulid-xyz` / `redis-type-os`). **Recorded as design + grammar continuity, not proven identity**: no payload was recovered from the new names, and none of the new publishers' artifacts (email `gradsherrod86@gmail.com`, publisher `whisdev`-class persona) join to the earlier cluster on public data.

**This wiki's registry state (~19:10 UTC Sep 22):** `oracle-redis` **LIVE** — created Sep 10, latest `6.1.2`, `legacy` tag `5.11.4`, flagged `5.11.3` directly installable, **65 downloads/week**. Carrier disposition is what disarms it: `redis-type-intel` was holder-created Sep 7 and now carries only `0.0.1-security` (no version satisfies `^1.10.5` → installing `oracle-redis@5.11.3` today fails resolution), and `pino-ulid` shows an explicit `unpublished` time block Sep 14 removing `2.12.2`/`2.12.3`/`3.0.1` — **delete-only, no security-holder, re-registerable**. The durable inversion of this page's own arming lesson: a deleted carrier is a **live fuse** — if `pino-ulid` 2.12.x is re-registered by whoever holds the account, the `oracle-redis` chain re-arms with no change to the live, downloading entry package. Watch/pin both names; hunt `oracle-redis` in lockfiles like you hunt `ioredis-xyz`; correlate any `pino-ulid` re-registration within minutes of `MicrosoftSystem64` persistence sightings.
