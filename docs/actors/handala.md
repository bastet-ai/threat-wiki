...
- **2026-09 (Group-IB attribution): the persona's malware named — HEAVYGRAM + CRUDEEXCLUDE.** [Group-IB](https://www.group-ib.com/blog/) attributes the **Handala Hack** persona to a Python-based Telegram-C2 surveillance backdoor (**HEAVYGRAM**, first detected in the wild September 2023) plus a Delphi-based Windows environment-prep utility (**CRUDEEXCLUDE**, first observed late July 2024). HEAVYGRAM built-in commands: remote command execution; system/network/process discovery; data and **Telegram session-file exfiltration**; screenshot capture; DLL sideloading; file cleanup; persistence via Windows autorun registry keys; on the Telegram channel it can also list programs, capture browser data, upload/download files, **turn on the microphone**, copy Telegram and WhatsApp data, and steal saved passwords. CRUDEEXCLUDE disguises itself as a GUI application and **configures Microsoft Defender exclusion paths** to stage later stages (incl. HEAVYGRAM), then runs PowerShell autorun persistence. UK NCSC tracks the same family as **CHOSEN BRICK** ("flexible tool... a range of potential operational outcomes"). Delivery: social engineering over **Telegram / WhatsApp / Instagram** — fake technical support or trusted-contact pretexts, payloads dressed as **Pictory, KeePass, and Telegram** installers. Target set per the FBI's March 2026 and September 2026 alerts: **Iranian dissidents, anti-regime journalists, and opposition groups** for intelligence collection, leaking, and reputational harm (builds on the Canadian RRM's September 2025 alert on doxxing of five Iran International journalists). Defender reads: the persona's toolkit is **surveillance-first, not wiper-first** — targeting journalists' messaging apps means Telegram session theft is the objective, so messengers on at-risk staff devices are the crown jewel; hunt Defender-exclusion-path additions + Telegram/WhatsApp-directory reads + autorun-key writes as one behavior cluster (Group-IB via The Hacker News, Sep 17, 2026; FBI alerts Mar + Sep 2026; NCSC CHOSEN BRICK).
...
## Sources
- The Hacker News, "Iran-Linked Handala Hack Tied to HEAVYGRAM Telegram Backdoor That Can Steal Passwords" (Sep 17, 2026 — relay of Group-IB attribution; source of HEAVYGRAM/CRUDEEXCLUDE tooling detail and the NCSC CHOSEN BRICK alias): <https://thehackernews.com/2026/09/iran-linked-handala-hack-tied-to.html>
